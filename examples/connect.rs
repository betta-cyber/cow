...
mod tcp {
    ...
    pub fn connect(
        ...
        let stream = Box::new(
            tcp.map(move |stream| {
                let (sink, stream) = BBBytes.framed(stream).split();
...
mod udp {
    ...
    pub fn connect(
        ...
        let (sink, stream) = UdpFramed::new(udp, BBBytes).split();
